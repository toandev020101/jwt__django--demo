...
const RoleManager = () => {
  ...
  useEffect(() => {
    ...
    const getPagination = async () => {
      setIsLoading(true);
      try {
        const res = await RoleApi.getPagination({
          _limit: rowsPerPage,
          _page: page,
          searchTerm,
        });
  ...
  }, [page, rowsPerPage, reload, isAuthenticated]);
  ...
  const handleSubmit = async (values) => {
    try {
      ...
      if (status === 400 || status === 403) {
        ...
        toast.error(`${openHandleDialog === 'add' ? 'Thêm' : 'Chỉnh sửa'} vai trò thất bại!`, {
          theme: 'colored',
          toastId: 'headerId',
          autoClose: 1500,
        });
  ...
  const handleDeleteRow = async () => {
    ...
    try {
      let res;
      if (deleteRowIndex === -1) {
        res = await RoleApi.removeAny({ ids: selectedArr });
      } else {
        res = await RoleApi.removeOne(rows[deleteRowIndex].id);
      }
  ...
  return (
    <Box>
      <TitlePage title="EngQuizz - Quản lý vai trò" />
      <Typography sx={{ marginBottom: '20px', fontSize: '18px' }}>Danh sách vai trò</Typography>
      {/* list content */}
      <Box
        padding="20px"
        marginBottom="30px"
      ...
      >
        <Header searchTerm={searchTerm} handleSearchChange={handleSearchChange} selectedArr={selectedArr}
                handleOpenAddDialog={handleOpenAddDialog} handleDeleteRowIndex={handleDeleteRowIndex} />
        ...
        <AddOrEditDialog form={form} openDialog={openHandleDialog}
                         handleClose={handleDialogClose}
                         handleSubmit={handleSubmit} />

        <RemoveDialog open={openDeleteDialog} onClose={handleDeleteDialogClose} title={'Xác nhận xóa vai trò'}
                      content={`Bạn chắc chắn muốn xóa ${deleteRowIndex === -1 ? selectedArr.length + ' vai trò này' : `vai trò "${rows[deleteRowIndex]?.name}"`} hay không?`}
